package com.hafiz.pareapp.ui.penyewa.detail
...
import kotlinx.android.synthetic.main.penyewa_activity_detail_produk.*
import kotlinx.android.synthetic.main.penyewa_activity_detail_produk.iv_produk
import kotlinx.android.synthetic.main.penyewa_content_detail_produk.*
...
class PenyewaDetailProdukActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.penyewa_activity_detail_produk)
    ...
    private fun setUI(){
        getPassedProduk()?.let {
            supportActionBar?.title = it.pemilik!!.nama_perusahaan
            iv_produk.load(it.foto)
            tv_harga.text = PareUtils.setToIDR(it.harga_sewa!!)
    ...
    private fun lanjutkan(){
        btn_order.setOnClickListener {
            startActivity(Intent(this@PenyewaDetailProdukActivity,PenyewaOrderActivity::class.java).apply {
                putExtra("PRODUCT", getPassedProduk())
            })
        }
    }
    ...
    private fun getPassedProduk() : Produk? = intent.getParcelableExtra("PRODUCT")
}
